...
class PaymentService:
    ...
    async def create_payment(self, amount: float, order_id: str) -> Dict[str, Any]:
        ...
        if amount < self.MIN_AMOUNT_RUB:
            logger.warning(f"Сумма {amount} ₽ меньше минимальной ({self.MIN_AMOUNT_RUB} ₽)")
            return {'error': f"Сумма заказа должна быть не менее {self.MIN_AMOUNT_RUB} ₽"}
        ...
        try:
            session = stripe.checkout.Session.create(
                ...
                mode='payment',
                success_url='https://t.me/Market3245bot?start=success_{}'.format(order_id),
                cancel_url='https://t.me/Market3245bot?start=cancel_{}'.format(order_id),
